  ...
  (b* ((op (binop-fix op)))
    (case op
      ...
      ((:eq_op :neq)
       (b* (((ev eq-res)
             (val-case v1
               :v_int (val-case v2 :v_int (ev_normal (eql v1.val v2.val))
                        :otherwise (ev_error "bad binop" (list op v1 v2)))
               :v_bool (val-case v2 :v_bool (ev_normal (eq v1.val v2.val))
                         :otherwise (ev_error "bad binop" (list op v1 v2)))
               :v_real (val-case v2 :v_real (ev_normal (equal v1.val v2.val))
                         :otherwise (ev_error "bad binop" (list op v1 v2)))
               :v_bitvector (val-case v2 :v_bitvector
                              (if (eql v1.len v2.len)
                                  (ev_normal (eql v1.val v2.val))
                                (ev_error "bad binop" (list op v1 v2)))
                              :otherwise (ev_error "bad binop" (list op v1 v2)))
               :v_string (val-case v2 :v_string (ev_normal (equal v1.val v2.val))
                           :otherwise (ev_error "bad binop" (list op v1 v2)))
               :v_label (val-case v2 :v_label (ev_normal (equal v1.val v2.val))
                          :otherwise (ev_error "bad binop" (list op v1 v2)))
               :otherwise (ev_error "bad binop" (list op v1 v2)))))
         (ev_normal (v_bool (xor (eq op :neq) eq-res)))))
      ((:leq :lt :geq :gt)
       (b* (((mv err val1 val2)
             (val-case v1
      ...
      ((:band :bor :beq :impl)
       (val-case v1 :v_bool
         ...
         :otherwise (ev_error "bad binop" (list op v1 v2))))
      (:rdiv (val-case v1
               :v_real (val-case v2 :v_real (if (< 0 v2.val)
                                                (ev_normal (v_real (/ v1.val v2.val)))
                                              (ev_error "bad rdiv operator" (list v1.val v2.val)))
                       :otherwise (ev_error "bad binop" (list op v1 v2)))
              :otherwise (ev_error "bad binop" (list op v1 v2))))
